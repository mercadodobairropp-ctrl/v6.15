...
function normalizarChecklist(c){const o={id:String(c.id||"").trim(),nome:String(c.nome||"").trim(),descricao:String(c.descricao||"").trim(),horario:normalizarHora(c.horario),horarioFim:normalizarHora(c.horarioFim||"23:59"),turnos:parseLista(c.turnos),dias:parseLista(String(c.dias||"").toLowerCase()),prioridade:String(c.prioridade||"media").toLowerCase(),responsaveisPermitidos:parseLista(c.responsaveisPermitidos||"").map(normalizarLogin),tarefas:parseTarefas(c.tarefas),ativo:String(c.ativo||"sim").toLowerCase(),empresaId:String(c.empresaId||"").trim()};if(!checklistCompleto(o))o.ativo="nao";return o}
...
async function carregarOperacionalOnline(login,empresaId,data=hojeISO(),force=false){
  ...
  return pendente(chave,async()=>{
    ...
    if(d?.status==="ok"||d?.status==="bloqueado"){
      ...
      Mem.checklists=(d.checklists||[]).map(normalizarChecklist).filter(c=>c.id&&c.nome);
      Mem.execucoes=d.execucoes||[];
      localStorage.setItem("usuariosSistema",JSON.stringify(Mem.usuarios));
      salvarCacheBase(d,agora);
...
function abrirBancoDispositivo(){return new Promise((resolve,reject)=>{if(!("indexedDB"in window)){reject(new Error("IndexedDB indisponível"));return}const req=indexedDB.open("checklistDispositivoV615",1);req.onupgradeneeded=()=>{const db=req.result;if(!db.objectStoreNames.contains("config"))db.createObjectStore("config",{keyPath:"key"})};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error||new Error("Falha ao abrir configuração do dispositivo"))})}
...
function abrirBancoUploads(){return new Promise((resolve,reject)=>{if(!("indexedDB"in window)){reject(new Error("IndexedDB indisponível"));return}const req=indexedDB.open("checklistArquivosV615",1);req.onupgradeneeded=()=>{const db=req.result;if(!db.objectStoreNames.contains("arquivos"))db.createObjectStore("arquivos",{keyPath:"id"})};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error||new Error("Falha ao abrir fila de arquivos"))})}
