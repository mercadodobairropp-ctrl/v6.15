...
function normalizarLogin_(login){
  const s=String(login == null ? "" : login).trim();
...
function criarAdminInicial_(){
  ...
  if(u.getLastRow()===1)u.appendRow(["1","1111","André","admin","",""]);
}
